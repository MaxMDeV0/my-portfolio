...
export default function List(props) {
    ...
    return (
        ...
        <section className={`flex flex-col py-10 px-4 space-y-7 items-center bg-${isBlack ? "black" : "white" } lg:py-[60px] min-[450px]:px-6 sm:px-10 md:px-16 lg:px-20 xl:px-24 2xl:px-28`}>
            ...
            {!!session && 
                <button onClick={()=>setIsEditing(true)} className={`h-16 w-16 p-4 rounded-full border-2 border-${!isBlack ? "black" : "white" }`}>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" fill={`${!isBlack ? "black" : "white" }`}>
